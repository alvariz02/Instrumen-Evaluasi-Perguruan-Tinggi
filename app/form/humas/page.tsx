...
import { formSchemaHumas, sectionKeysHumas } from '@/lib/formSchema';
...
export default function HumasForm() {
  ...
  const progress = calculateFormProgress(formSchemaHumas, headerData, formData);

  useEffect(() => {
    const savedData = localStorage.getItem('humasFormDraft');
    if (savedData) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('humasFormDraft', JSON.stringify({
      headerData,
      formData,
      submissionId,
    }));
  ...
  const handleSubmit = async () => {
    ...
    Object.values(formSchemaHumas).forEach((section) => {
    ...
    try {
      ...
      localStorage.removeItem('humasFormDraft');
      setHeaderData({
        institution_name: '',
        submitted_by: '',
        position: '',
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            Instrumen Hubungan Masyarakat (HUMAS)
          </h1>
          <p className="text-gray-600">
            Silakan isi formulir evaluasi unit HUMAS di bawah ini dengan lengkap dan benar.
          </p>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          {sectionKeysHumas.map((key) => (
            <FormSection
              key={key}
              sectionKey={key}
              section={formSchemaHumas[key]}
              data={formData}
          ...
          <div className="flex gap-4 mt-8">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 bg-green-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-green-700 disabled:bg-green-400 transition-colors"
            >
