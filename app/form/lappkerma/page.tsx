...
import { formSchemaLappkerma, sectionKeysLappkerma } from '@/lib/formSchema';
...
export default function LappkermaForm() {
  ...
  const progress = calculateFormProgress(formSchemaLappkerma, headerData, formData);

  useEffect(() => {
    const savedData = localStorage.getItem('lappkermaFormDraft');
    if (savedData) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem(
      'lappkermaFormDraft',
      JSON.stringify({ headerData, formData, submissionId })
    );
  ...
  const handleSubmit = async () => {
    ...
    Object.values(formSchemaLappkerma).forEach((section) => {
...
BIDANG KERJASAMA PERGURUAN TINGGI
          ...
          <p className="text-gray-600">
            Silakan isi formulir terkait upload dokumen kerja sama ke laman LAPPKERMA.
          </p>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          {sectionKeysLappkerma.map((key) => (
            <FormSection
              key={key}
              sectionKey={key}
              section={formSchemaLappkerma[key]}
              data={formData}
